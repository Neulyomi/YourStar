 ...
 * - 366일(1월 1일 ~ 12월 31일) 전 일정 고유 천문 좌표(RA, Dec) 및 JWST 실사 컬렉션 매핑
 * - [우주의 모든 별 활성화 모드] 시 366개 전체 생일 천체가 360도 밤하늘 천구에 압도적으로 전개
 */

const JWST_MASTER_TEMPLATES = [
  {
    nameTemplate: "카리나 성운 '우주의 절벽'",
    catalogBase: "NGC 3324 / JWST NIRCam",
    type: "JWST 별 탄생 성운",
  ...
  {
    nameTemplate: "독수리 성운 '창조의 기둥'",
    catalogBase: "M16 / NGC 6611 (JWST)",
  ...
  {
    nameTemplate: "남쪽 고리 성운 (Southern Ring)",
    catalogBase: "NGC 3132 / JWST MIRI",
    type: "JWST 행성상 성운",
  ...
  {
    nameTemplate: "타란툴라 성운 (30 Doradus)",
    catalogBase: "NGC 2070 / JWST NIRCam",
  ...
  {
    nameTemplate: "스테판의 5중주 (Stephan's Quintet)",
    catalogBase: "HCG 92 / JWST NIRCam",
    type: "JWST 상호작용 은하군",
  ...
  {
    nameTemplate: "SMACS 0723 딥 필드",
    catalogBase: "Webb's First Deep Field",
  ...
  {
    nameTemplate: "차륜 은하 (Cartwheel Galaxy)",
    catalogBase: "ESO 350-40 / JWST",
    type: "JWST 충돌 고리 은하",
  ...
  {
    nameTemplate: "판도라 성단 (Pandora's Cluster)",
    catalogBase: "Abell 2744 (JWST)",
  ...
  {
    nameTemplate: "고리 성운 (Ring Nebula)",
    catalogBase: "M57 / NGC 6720 (JWST)",
    type: "JWST 행성상 성운",
  ...
  {
    nameTemplate: "말머리 성운 (Horsehead Nebula)",
    catalogBase: "Barnard 33 (JWST NIRCam)",
    type: "JWST 적외선 암흑 성운",
...
class NASAStarDatabase {
  constructor() {
    this.catalog = [];
    this.generateFullYearCatalog();
  }

  /**
   * 366일(1.1 ~ 12.31) 전체 날짜별 고유 천문 좌표 및 JWST 천체 데이터베이스 빌드
   */
  generateFullYearCatalog() {
    ...
    for (let m = 1; m <= 12; m++) {
      ...
      for (let d = 1; d <= maxDays; d++) {
        globalDayIndex++;
        const tmplIndex = (globalDayIndex - 1) % JWST_MASTER_TEMPLATES.length;
        const tmpl = JWST_MASTER_TEMPLATES[tmplIndex];

        // 360도 천구 전역(RA: 0~24h, Dec: -75° ~ +75°)에 황금비(Golden Spiral) 기반으로 고르게 분산
        const phi = (globalDayIndex * 2.399963229728653) % (Math.PI * 2); // 황금 각도
        const raHours = (phi / (Math.PI * 2)) * 24;
        const decNormalized = (globalDayIndex / 366) * 2 - 1; // -1 ~ +1
        const decDegrees = Math.asin(decNormalized) * (180 / Math.PI) * 0.85; // 약 -72° ~ +72°

        const starEntry = {
          dayOfYear: globalDayIndex,
          month: m,
          day: d,
          birthDayText: `${m}월 ${d}일`,
          name: `${tmpl.nameTemplate} (${m}월 ${d}일의 탄생 천체)`,
          catalog: `${tmpl.catalogBase} • #${globalDayIndex}`,
          type: tmpl.type,
          constellation: tmpl.constellation,
          ra: Number(raHours.toFixed(3)),
          dec: Number(decDegrees.toFixed(2)),
          distance_ly: tmpl.distance_ly,
          distance_num: tmpl.distance_num,
          history_story: tmpl.history_story,
          orbital_speed: tmpl.orbital_speed,
          galactic_period: tmpl.galactic_period,
          image: tmpl.image,
          story: tmpl.story
        };

        this.catalog.push(starEntry);
      }
    }
  }
  ...
  getStarByDate(month, day, preferVisible = false, astroEngine = null) {
    ...
    const primaryStar = this.catalog.find(s => s.month === month && s.day === day) || this.catalog[dayOfYear - 1];

    if (preferVisible && astroEngine) {
      const primaryHoriz = astroEngine.equatorialToHorizontal(primaryStar.ra, primaryStar.dec);
      if (primaryHoriz.altitude > 10) {
        return Object.assign({}, primaryStar, { isVisiblePriorityMatch: true });
      }
      ...
      for (let i = 0; i < this.catalog.length; i++) {
        ...
        if (horiz.altitude > 15) {
          return Object.assign({}, star, { isVisiblePriorityMatch: true, requestedBirthday: `${month}월 ${day}일` });
        }
      ...
      if (bestStar) {
        return Object.assign({}, bestStar, { isVisiblePriorityMatch: false, requestedBirthday: `${month}월 ${day}일` });
      }
    }

    return Object.assign({}, primaryStar, { isVisiblePriorityMatch: false });
  }

 ...
 * - [1.5배 초원거리 웅장 천구 돔 매핑: R = 570]
 * - 원근 왜곡(Perspective Distortion) 완전 배제 및 360도 천구 파노라마
...
class StellarEngine {
  ...
  init() {
    ...
    // 1. 씬 & 초원거리 원근 카메라 (near: 0.5, far: 6000)
    this.scene = new THREE.Scene();
    this.camera = new THREE.PerspectiveCamera(50, width / height, 0.5, 6000);
    ...
    // 4. 배경 심우주 별무리 (2,800개, R = 1200~2000)
   ...
   */
  createThickLineMesh(p1, p2, radius = 1.8, colorHex = 0x38bdf8, opacity = 0.65) {
  ...
  /**
   * [1.5배 원거리 지평선 나침반 그리드: R = 570]
   */
  buildHorizonGrid() {
    const radius = 570;
    
    // 3px 굵기 나침반 원형 튜브 토러스 메쉬
    const torusGeo = new THREE.TorusGeometry(radius, 1.35, 6, 128);
    ...
    for (let deg = 0; deg < 360; deg += 45) {
      ...
      const innerPt = new THREE.Vector3((radius - 12.0) * Math.sin(rad), 0, -(radius - 12.0) * Math.cos(rad));
      const outerPt = new THREE.Vector3((radius + 12.0) * Math.sin(rad), 0, -(radius + 12.0) * Math.cos(rad));
      const tickMesh = this.createThickLineMesh(innerPt, outerPt, 1.2, 0x38bdf8, 0.70);
    ...
    for (let deg = 0; deg < 360; deg += 15) {
      ...
      const labelDist = radius + (isMajor ? 18.0 : 12.0);
  ...
  /**
   * [1.5배 원거리 별자리 튜브 선 & 별자리 항성 렌더링: R = 570]
   */
  renderConstellations(astroEngine, activeZodiacId = null) {
    ...
    window.CONSTELLATIONS_DATA.forEach(constellation => {
      ...
      const tubeRadius = isHighlighted ? 2.2 : 1.5;

      const starWorldPositions = [];

      constellation.stars.forEach((s) => {
        const cart = astroEngine.getCartesianFromEquatorial(s.ra, s.dec, 570);
        ...
        const starScale = isHighlighted ? 20.0 : 15.0;
        starSprite.scale.set(starScale, starScale, 1);
        this.constellationGroup.add(starSprite);

        const starHitGeo = new THREE.SphereGeometry(65.0, 8, 8);
  ...
  /**
   * [1.5배 원거리 생일 별 & 24시간 공전 궤도선 렌더링: R = 570]
   */
  setupStellarObject(starData, astroEngine) {
    ...
    const orbitRadius = 570;
    const cart = astroEngine.getCartesianFromEquatorial(starData.ra, starData.dec, orbitRadius);
    const starPos = new THREE.Vector3(cart.x, cart.y, cart.z);

    // 24시간 일주 운동 궤적 튜브
    const orbitPoints = astroEngine.getDiurnalOrbitPath(starData.ra, starData.dec, orbitRadius, 96);
    ...
    const orbitTubeGeo = new THREE.TubeGeometry(this.fullOrbitCurve, 128, 1.8, 8, true);
    ...
    const hitGeo = new THREE.SphereGeometry(110.0, 8, 8);
  ...
  renderAllStellarObjects(allStarsList, astroEngine) {
    ...
    allStarsList.forEach((starData) => {
      ...
      const haloGeo = new THREE.RingGeometry(photoScale * 0.54, photoScale * 0.58, 24);
      const haloMat = new THREE.MeshBasicMaterial({
        color: 0x38bdf8,
        transparent: true,
        opacity: 0.70,
      ...
      const hitGeo = new THREE.SphereGeometry(36.0, 6, 6);
      const hitMat = new THREE.MeshBasicMaterial({ transparent: true, opacity: 0, depthWrite: false });
      const hitMesh = new THREE.Mesh(hitGeo, hitMat);
      hitMesh.userData = { isPrimeStar: true, starData: starData };
      container.add(hitMesh);
